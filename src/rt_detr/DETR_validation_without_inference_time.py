...
def count_predictions_with_conf(labels_dir, conf_threshold):
    """
    Count predicted instances in labels_dir whose confidence ≥ conf_threshold.
    Each .txt line: <class> <x> <y> <w> <h> <confidence>
    We need to filter here because RTDETR.val does not apply a confidence threshold to .txt outputs.
    """
    count = 0
    if os.path.isdir(labels_dir):
        for fname in os.listdir(labels_dir):
            ...
            with open(os.path.join(labels_dir, fname), "r") as f:
                for line in f:
                    ...
                    # expect at least 6 elements, last is confidence
                    if len(parts) >= 6:
                        try:
                            conf = float(parts[-1])
                        except ValueError:
...
def run_and_process_inference(model_path, data, imgsz, device, project, run_name,
    ...
    # Assuming save_params is defined earlier in the script
    conf_threshold = save_params.get('conf', 0.0)  # Default to 0.0 if 'conf' is not set
    # Pass the confidence threshold to the function
    predicted_count = count_predictions_with_conf(pred_labels_dir, conf_threshold)
    if predicted_count:
...
if __name__ == "__main__":
    ...
    imgsz = 1280
    device = "cuda"
    project = "validation_results_RT_DETR_without_inference_time"
    run_name = "RT_DETR_Validation_for_single_class"
