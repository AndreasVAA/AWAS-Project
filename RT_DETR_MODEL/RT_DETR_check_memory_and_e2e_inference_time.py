...
MODEL_PATH   = "/home/itk/Desktop/Andreas/AWAS-Project/RT_DETR_MODEL/Testing_RT_DETR_variations_patience_20/RT_DETR_model_sizeX_640_batch4/weights/best.pt"  # update with your model path
...
OUTPUT_CSV   = "RT_DETR_model_x_640_benchmark_results.csv"
...
def predict_rt_detr(inputs):
    """
    inputs: single image path (str) or list of paths
    """
    return rt_detr.predict(
        source=inputs,
...
def run_benchmark():
    global rt_detr
    rt_detr = RTDETR(MODEL_PATH)
    _ = rt_detr.model.to(DEVICE).eval()
    ...
    for _ in range(WARMUP_ITERS):
        _ = predict_rt_detr(warmup_inputs)
    ...
    if BATCH_MODE:
        for idx in range(0, len(img_paths), BATCH_SIZE):
            ...
            t_ms, mem_mb, _ = time_full_prediction(predict_rt_detr, batch)
            run_id = idx // BATCH_SIZE
            results.append((run_id, t_ms, mem_mb))
    else:
        for idx, img_path in enumerate(img_paths):
            t_ms, mem_mb, _ = time_full_prediction(predict_rt_detr, img_path)
